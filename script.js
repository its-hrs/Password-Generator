...
function calcStrength() {
    ...
    if (hasUpper && hasLower && (hasNumber || hasSymbol) && passwordLength >= 8) {
        ...
        level.style.color = '#0f0';
        return '#0f0';
    } 
    else if ((hasLower || hasUpper) && (hasNumber || hasSymbol) && passwordLength >= 6) {
        setIndicator("#ff0");
        level.innerText = 'Medium';
        level.style.color = '#ff0';
        return '#ff0';
    } 
    else {
        setIndicator("#f00");
        level.innerText = 'Weak';
        level.style.color = '#f00';
        return '#f00'
    }
}
...
// Copy Message 
let copyMessage = document.querySelector(".copied");
...
generateBtn.addEventListener('click', () => {
    generateBtn.classList.add('hover');
    setTimeout(() => {
        generateBtn.classList.remove('hover');
    },100);

    if (checkCount <= 0){
        alert('Please select any of the given checkboxes!');
        return;
    }
    ...
    let col = calcStrength();
    passwordDisplay.style.cssText = `color:${col}`;
});